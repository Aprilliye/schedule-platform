<template>
    <div class="container">
        <div class="content-header">
            ...
            <a class="btnDefault bgGreen" @click="modal.addShift=true" >新增班制qqq</a>
        </div>
        ...
        <Modal title="新增班次111"
            ...
            <Form ref="addFormValidateClass" :model="addFormValidateClass" :rules="ruleValidate1" :label-width="80">
                ...
                <FormItem label="起止时间" prop="timeSlot" element-id="timeSlot">
                    ...
                    <TimePicker  v-model="addFormValidateClass.timeSlotBegin" placeholder="选择开始时间" format="HH:mm" ></TimePicker>至
                    <TimePicker  v-model="addFormValidateClass.timeSlotEnd" placeholder="选择结束时间" format="HH:mm" ></TimePicker>
                </FormItem>
        ...
        <Modal title="编辑班次"
            ...
            <Form ref="editFormValidateClass" :model="editFormValidateClass" :rules="ruleValidate1" :label-width="80">
                ...
                <!-- <FormItem label="班次关联" prop="relevantClassId">
                      <Select v-model="editFormValidateClass.relevantClassId" placeholder="请选择">
                        <Option v-for="(item,index) in CurrentRelevantClassId" :value="item.id" :key="index">{{item.districtName}}</Option>
                    </Select> -->
...
export default {
    ...
    methods:{
        ...
        beforeEditShifyClassMethods: async function (that) {
                let suiteId = that.suiteId;
                let result = await getClass(suiteId);
                console.log(result);
                let currentPosition = that.position.current.split('-');
                let data={
                    id: that.classId,
